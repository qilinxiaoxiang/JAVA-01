...
## 4. 对于不同GC和堆内存的总结
1. 理论(零阶道理)
	1. SerialGC单线程, 如果发生fullGC, 则内存越大, STW越长.
	2. ParallelGC是SerialGC的多线程版, 吞吐量达, 如果内存越大, STW也越长.
	3. CMS吞吐量比ParallelGC小, STW也比ParallelGC短.
	4. G1在大内存下, STW最短, gc逻辑最复杂, 所以吞吐量相比CMS可能更小一些.
2. 实际(一阶修正)
	1. **G1并不一定比SerialGC好, 不管是大内存还是小内存**, 尤其是不发生fullGC的场景.
	2. **ParallelGC也并不一定吞吐量达, 不管是大内存还是小内存.**
	3. **内存大性能未必好.**
	4. **比来比去, 发现不同GC的差别根本就不大, 如果不是有特殊要求或者遇到特殊问题, 确实没有必要优化GC.**
	5. **即便要进行优化, 也要实事求是地进行压测确定.**
